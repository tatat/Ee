...
define([], function() {
  ...
  Ee.prototype.on = function(event_types, listeners, times, first, until) {
    if (!__is_array(event_types))
      event_types = [event_types];
    ...
    for (var i = 0, j = event_types.length, ii; i < j; i ++) {
      ...
      for (ii = 0; ii < length; ii ++) {
        ...
        if (new_listener)
          this.emit('newListener', event_type, listener, times, first);
      }
    }
  ...
  Ee.prototype.once = function(event_types, listeners, first, until) {
    return this.on(event_types, listeners, 1, first, until);
  };

  Ee.prototype.first = function(event_types, listeners, times, until) {
    return this.on(event_types, listeners, times, true, until);
  };

  Ee.prototype.until = function(until, event_types, listeners, times, first) {
    return this.on(event_types, listeners, times, first, until);
  };
  ...
  Ee.prototype.until_mutually = function(event_types, listeners, times, first) {
    for (var i = 0, j = event_types.length, event_type, ii, until; i < j; i ++) {
      for (until = [], event_type = event_types[i], ii = 0; ii < j; ii ++)
        event_type !== event_types[ii] && until.push(event_types[ii]);
      
      this.on(event_type, listeners, times, first, until);
    }

    return this;
  };

  Ee.prototype.until_once_mutually = function(event_types, listeners, first) {
    return this.until_mutually(event_types, listeners, 1, first);
  };

  Ee.prototype.within = function(ms, event_types, listeners, callback, times, first, until) {
    if (typeof ms === 'object' && ms) {
      var options = ms;
    ...
    var self = this
      , last_listeners = this.on(event_types, listeners, times, first, until).__listeners;
