...
define([], function() {
  ...
  Ee.prototype.until_mutually = function(event_types, listeners, until, times, first) {
    ...
    for (var i = 0, j = event_types.length, event_type, ii, _until; i < j; i ++) {
      ...
      _until.length && this.on(event_type, listeners, times, first, _until);
    }
  ...
  Ee.prototype.within_once = function(ms, event_types, listeners, callback, first, until) {
    if (ms && typeof ms === 'object')
      ms.times = 1;

    return this.within(ms, event_types, listeners, callback, 1, first, until);
  };
